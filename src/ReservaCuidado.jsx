    ...
    <div className="bg-[#F2F7FD] min-h-screen pb-10">
      ...
      <main className="px-5 max-w-[500px] mx-auto pt-[88px]">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="m-0 text-black text-[16px] leading-[22px] font-bold">Reserva de Cuidado</h2>
        </div>

        <div className="flex items-end justify-between mb-5">
          <p className="m-0 text-[#9A9A9A] italic text-[16px] leading-[21px] max-w-[290px]">
            “Las mascotas son recogidas a las 8:00 am y dejadas a las 6:00pm.”
          </p>
          <Icono name="iconhuelladoble" alt="huellas" className="w-[31px] h-[31px] opacity-80" />
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <DatePicker
            ...
            onChange={(date) => {
              setFechaRecojo(date);
              if (fechaFin && date && fechaFin < date) setFechaFin(null);
            }}
            locale="es"
          ...
          <DatePicker
            ...
            locale="es"
            dateFormat="dd/MM/yyyy"
            placeholderText="Fecha/fin del servicio"
            filterDate={(date) => !estaBloqueada(date)}
            minDate={fechaRecojo || new Date()}
            showYearDropdown
            showMonthDropdown
            dropdownMode="select"
            yearDropdownItemNumber={2}
          ...
          <div className="h-[78px] bg-white rounded-[18px] px-6 flex items-center gap-4 shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
            <Icono
              name="iconsoles"
              alt="monto"
              className="w-8 h-8"
